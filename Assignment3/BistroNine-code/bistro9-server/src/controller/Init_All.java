 ...
 * Aligned with DataBaseController schema.
 */
public class Init_All {
    ...
    // --- פונקציה חדשה להוספת שעות מיוחדות ---
    private static void initSpecialHours(Connection con, Statement stmt) {
        try {
            System.out.println("Initializing special hours...");

            // --- דצמבר 2025 (היסטוריה קרובה) ---
            ...
            // --- ינואר 2026 (עתיד קרוב) ---
            
            // 1/1: חגיגות שנה חדשה (פיצול לילה ויום)
    ...
    private static void createTables(Connection con, Statement stmt) {
        try {
            ...
            stmt.executeUpdate("CREATE TABLE subscriber (" +
            ...
            stmt.executeUpdate("CREATE TABLE restaurant_tables (" +
            ...
            stmt.executeUpdate("CREATE TABLE table_reservations (" +
            ...
            stmt.executeUpdate("CREATE TABLE weekly_hours (" +
            ...
            stmt.executeUpdate("CREATE TABLE special_hours (" +
                    "specificDate DATE NOT NULL, " +
                    "openingTime TIME NOT NULL, " +
                    "closingTime TIME NOT NULL, " +
                    "PRIMARY KEY (specificDate, openingTime));");

            stmt.executeUpdate("CREATE TABLE restaurant_discount (" +
            ...
            stmt.executeUpdate("CREATE TABLE waiting_list (" +
            ...
            stmt.executeUpdate("CREATE TABLE report_manager (" +
            ...
            stmt.executeUpdate("CREATE TABLE time_report (" +
            ...
            stmt.executeUpdate("CREATE TABLE subscriber_report (" +
                    "reportId INT NOT NULL, " +
                    "reportDate DATE NOT NULL, " +
    ...
    private static void initDiscounts(Connection con, Statement stmt) {
        try {
            ...
            System.out.println("Initialized discounts.");
        } catch (SQLException e) { e.printStackTrace(); }
    }

    private static void initOpeningHours(Connection con, Statement stmt) {
        try {
            String[] weekDays = { "SUNDAY", "MONDAY", "WEDNESDAY", "THURSDAY" };
            ...
            stmt.executeUpdate("INSERT INTO weekly_hours (dayOfWeek, openingTime, closingTime) VALUES ('SATURDAY', '20:00:00', '03:00:00')");
            System.out.println("Inserted default weekly hours.");
        } catch (SQLException e) { e.printStackTrace(); }
    }
    ...
    private static void initTables(Connection con, Statement stmt) {
        try {
            ...
            System.out.println("Inserted 10 restaurant tables (Sizes: 2, 4, 6).");
        } catch (SQLException e) { e.printStackTrace(); }
    }

    private static void initUsersAndSubscribers(Connection con) {
        String[] firstNames = { "roi", "may", "Daniel", "eli", "John", "Jennifer", "Michael", "Linda", "David", "Sarah" };
        ...
        try (PreparedStatement psCust = con.prepareStatement(insertCust, Statement.RETURN_GENERATED_KEYS);
            ...
            // 2. יצירת נציג מסעדה (Restaurant Representative)
            ...
            // 4. לקוחות מזדמנים (Regular Customers) - ללא משתמש ב-Subscriber
            
            // קבוצה א': גם טלפון וגם אימייל (3 לקוחות)
            for (int k = 1; k <= 3; k++) {
            ...
            // קבוצה ב': רק טלפון (אימייל NULL) (3 לקוחות)
            for (int k = 1; k <= 3; k++) {
                psCust.setString(1, "054222222" + k);
                // שימוש ב-setNull עבור האימייל
                psCust.setNull(2, java.sql.Types.VARCHAR);
                psCust.executeUpdate();
            }

            // קבוצה ג': רק אימייל (טלפון NULL) (3 לקוחות)
            for (int k = 1; k <= 3; k++) {
                // שימוש ב-setNull עבור הטלפון
                psCust.setNull(1, java.sql.Types.VARCHAR);
                psCust.setString(2, "email_only_guest_" + k + "@gmail.com");
                psCust.executeUpdate();
            }

            System.out.println("Inserted: Manager, Rep, Subscribers, and Mixed Regular Customers (Full/Phone-Only/Email-Only).");
        } catch (SQLException e) { e.printStackTrace(); }
    }
    ...
    private static void initReservationsAndBills(Connection con) {
        ...
        String insertRes = "INSERT INTO table_reservations (tableId, numberOfDiners, confirmationCode, customerId, reservationDate, arrivalTime, leavingTime, status) " +
                           "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        
        String insertBill = "INSERT INTO bills (reservationId, totalAmount, totalAmountAfterDiscount, discountPercentage, isPaid, discountType, paymentMethod) " +
                            "VALUES (?, ?, ?, ?, true, ?, 'credit')";
        ...
        try (Statement stmt = con.createStatement();
            ...
            System.out.println("Found " + custIds.size() + " customers. Generating reservations (Mixed: Completed & Cancelled)...");
            ...
            System.out.println("Reservations populated (Dec 25 + Jan 26) with ~20% cancellation rate.");
        } catch (SQLException e) { e.printStackTrace(); }
    }
    ...
    private static void createDailyReservations(Connection con, PreparedStatement psRes, PreparedStatement psBill, List<Integer> custIds, LocalDate date, int startConfCode) throws SQLException {
        ...
        for (int i = 0; i < dailyRes; i++) {
            ...
            LocalDateTime start = LocalDateTime.of(date, LocalTime.of(12 + (i % 8), 0));
            LocalDateTime end = start.plusHours(2);

            // --- לוגיקה לביטולים ---
            // נניח שיש סיכוי של 20% שההזמנה בוטלה
            ...
            psRes.setTimestamp(5, Timestamp.valueOf(start));
            
            // אם בוטלה, הזמנים בפועל עדיין נרשמים כשעות שהיו אמורות להיות (או שאפשר לשים null, תלוי בלוגיקה שלך)
            // כאן נשאיר אותם כדי לא ליצור בעיות בשאילתות, אבל הסטטוס קובע.
            psRes.setTimestamp(6, Timestamp.valueOf(start.plusMinutes(5)));
            psRes.setTimestamp(7, Timestamp.valueOf(end));
            
            psRes.setString(8, status); // שימוש בסטטוס שהגרלנו
            
            psRes.executeUpdate();

            // --- יצירת חשבונית (רק אם לא בוטלה) ---
            if (!isCancelled) {
    ...
    private static void initReports(Connection con, Statement stmt) {
        try {
            String insertReport = "INSERT INTO report_manager (startDay, endDay, reportRange, reportType) VALUES ('2025-11-01', '2025-11-30', 'monthly', 'time')";
            stmt.executeUpdate(insertReport, Statement.RETURN_GENERATED_KEYS);
            ...
            insertReport = "INSERT INTO report_manager (startDay, endDay, reportRange, reportType) VALUES ('2025-11-01', '2025-11-30', 'monthly', 'subscriber')";
            stmt.executeUpdate(insertReport, Statement.RETURN_GENERATED_KEYS);
            ...
            System.out.println("Initialized dummy reports.");
        } catch (SQLException e) { e.printStackTrace(); }
    }
}